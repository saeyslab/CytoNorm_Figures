---
title: "Figures CytofNorm Paper"
output: html_notebook
---

# Figures CytofNorm Paper
...
for(exp in rownames(exp_setups)){
  ...
  saveRDS(model,
          file = file.path(results,
                           paste0("GatesExtraControls_CytofNorm_", exp, ".rds")))
}
```
...
for(exp in rownames(exp_setups)){
  ...
  t <- system.time(capture.output(
    CytofNorm.normalize(model = model,
                        ...
                        outputDir = paste0("CytofNorm_", exp),
...
# Figure 4
```{r}
plot_emds("CytofNorm_Quantile_1_Unstim", 
          "Original_1_Unstim",
          emds_g)
```
...
wilcox.test(emds$Original_1_Unstim, 
            emds$CytofNorm_Quantile_1_Unstim,
            paired = TRUE)
```
...
for(exp in rownames(exp_setups)){
  exp_original <- gsub(".*_([12].*)", "Original_\\1", exp)
  print(plot_emds(exp, exp_original, emds_g))
}
dev.off()
```
...
```{r, fig.width = 12}
p1 <- plot_emds("CytofNorm_Quantile_1_Unstim", 
               "QuantileNorm_1_Unstim",
               emds_g)

p2 <- plot_emds("CytofNorm_MinMax_1_Unstim", 
          "MinMaxNorm_1_Unstim",
          emds_g)
...
pheatmap::pheatmap(t(emds_g[, #rownames(emds_g)[order(emds_g[,"Original_1_Unstim"])],  
                            c("Original_1_Unstim",
                               ...
                               "CytofNorm_MinMax_1_Unstim",
                               "CytofNorm_MinMax_2_Unstim",
                               "CytofNorm_MinMax_1_IFNa_LPS",
                               "CytofNorm_MinMax_2_IFNa_LPS",
                               "QuantileNorm_1_Unstim",
                               "QuantileNorm_2_Unstim",
                               "QuantileNorm_1_IFNa_LPS",
                               "QuantileNorm_2_IFNa_LPS",
                               "CytofNorm_Quantile_1_Unstim",
                               "CytofNorm_Quantile_2_Unstim",
                               "CytofNorm_Quantile_1_IFNa_LPS",
                               "CytofNorm_Quantile_2_IFNa_LPS")]),
                   cluster_cols = FALSE,
                   cluster_rows = FALSE,
...
```{r}
norm_dirs <- paste0("CytofNorm_", c(2,1)[df$Volunteer], "_", df$Stim, "_Quantile")
...
```{r}
exp_setups <- expand.grid(method = c("CytofNorm_Quantile"),
                          volunteer = 1,
                          stim = c("Unstim", "IFNa_LPS"))
rownames(exp_setups) <- apply(exp_setups, 1, paste, collapse = "_")
...
for(volunteer in c("1","2")){
  ...
  saveRDS(model,
          file = file.path(results,
                           paste0("GatesExtraControls_CytofNorm_", volunteer, "_Both_Quantile.rds")))
}
```

```{r}
model <- readRDS(file.path(results,
                            paste0("GatesExtraControls_CytofNorm_1_Both_Quantile.rds")))
...
t <- system.time(capture.output(
  CytofNorm.normalize(model = model,
                      ...
                      transformList = transformList,
                      transformList.reverse = transformList.reverse,
                      outputDir = paste0("CytofNorm_1_Both_Quantile"),
                      normMethod.normalize = QuantileNorm.normalize)))
print(t)
```
...
for (stim in c("Unstim", "IFNa_LPS")) {
  ...
  o <- capture.output(suppressWarnings(
    emds_both_stim[[stim]] <- emdEvaluation(file.path("CytofNorm_1_Both_Quantile", 
                                                      paste0("Norm_",
                                                             toEvaluate$File)),
                                 transformList = transformList,
                                 manual = lapply(manual, function(x){x$manual}),
...
p2 <- plot_emds("Other_stimCytofNorm_Quantile_1_IFNa_LPS",
                "Original_1_Unstim",
                emds_all_stim_g,
                title = "Unstimulated samples V2",
...
p4 <- plot_emds("Other_stimCytofNorm_Quantile_1_Unstim",
                "Original_1_IFNa_LPS",
                emds_all_stim_g,
                title = "Stimulated samples V2",
                subtitle = "trained on unstimulated samples V1")

p5 <- plot_emds("CytofNorm_Quantile_1_IFNa_LPS",
                "Original_1_IFNa_LPS",
                emds_all_stim_g,
                title = "Stimulated samples V2",
                subtitle = "trained on stimulated samples V1")
...
legend("topright", 
       legend = c("Original", "Quantile normalization", "CytofNorm normalization"), 
...
suppressMessages(o <- capture.output(
  fs_n <- read.ncdfFlowSet(file.path("CytofNorm_1_Unstim_Quantile", 
...
fs_norm <- read.ncdfFlowSet(file.path("CytofNorm_1_IFNa_LPS_Quantile", 
